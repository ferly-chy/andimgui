...
// 反射 ActivityThread.mActivities 找到一个非 finishing 的 Activity。
// 成功返回 LocalRef（位于调用方 PushLocalFrame 内）；失败返回 nullptr。
// 任何 android.app.Activity 子类均接受，不限 NativeActivity。
