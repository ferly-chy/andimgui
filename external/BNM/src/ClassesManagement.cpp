...
static inline void SafePatchVTable(BNM::IL2CPP::VirtualInvokeData *vtable,
                                   void *newMethodPtr,
                                   const BNM::IL2CPP::MethodInfo *newMethod) {
  // C++20 std::atomic_ref is the correct way to perform atomic operations on
  // non-atomic types
  std::atomic_ref<const BNM::IL2CPP::MethodInfo *> methodRef(vtable->method);
  methodRef.store(newMethod, std::memory_order_release);

  std::atomic_ref<BNM::IL2CPP::Il2CppMethodPointer> ptrRef(vtable->methodPtr);
  ptrRef.store((BNM::IL2CPP::Il2CppMethodPointer)newMethodPtr,
...
#define BNM_I2C_NEW(type)                                                      \
  (IL2CPP::type *)BNM_MALLOC_TRACKED(sizeof(IL2CPP::type))
...
static void CreateClass(MANAGEMENT_STRUCTURES::CustomClass *customClass,
                        const CustomClassInfo &classInfo);
...
static IL2CPP::MethodInfo *
ProcessCustomMethod(MANAGEMENT_STRUCTURES::CustomMethod *method, Class target,
                    bool *hooked = nullptr);
// The code for creating new methods
static IL2CPP::MethodInfo *
CreateMethod(MANAGEMENT_STRUCTURES::CustomMethod *method);
// The code for setting data in fields
static void SetupField(IL2CPP::FieldInfo *newField,
                       MANAGEMENT_STRUCTURES::CustomField *field);

// Code for getting all interfaces of class parent and interfaces
static void GetAllInterfaces(IL2CPP::Il2CppClass *parent,
                             IL2CPP::Il2CppClass *interface,
                             std::vector<IL2CPP::Il2CppClass *> &outInterfaces);
...
static void SetupClassParent(IL2CPP::Il2CppClass *target,
                             IL2CPP::Il2CppClass *parent);
// Code for checking whether a class and its parents have an interface
static bool HasInterface(IL2CPP::Il2CppClass *parent,
                         IL2CPP::Il2CppClass *interface);
...
void ClassesManagement::ProcessClassRuntime(
  ...
  if (info._class)
    ModifyClass(customClass, info._class);
  else if (info._name)
    CreateClass(customClass, info);
  else {
#ifdef BNM_DEBUG
...
static size_t GetTypeAlignment(const IL2CPP::Il2CppType *type) {
  if (!type)
    return sizeof(void *);
  switch (type->type) {
  case IL2CPP::IL2CPP_TYPE_I1:
  case IL2CPP::IL2CPP_TYPE_U1:
  case IL2CPP::IL2CPP_TYPE_BOOLEAN:
    return 1;
  case IL2CPP::IL2CPP_TYPE_I2:
  case IL2CPP::IL2CPP_TYPE_U2:
  case IL2CPP::IL2CPP_TYPE_CHAR:
    return 2;
  case IL2CPP::IL2CPP_TYPE_I4:
  case IL2CPP::IL2CPP_TYPE_U4:
  case IL2CPP::IL2CPP_TYPE_R4:
    return 4;
  case IL2CPP::IL2CPP_TYPE_I8:
  case IL2CPP::IL2CPP_TYPE_U8:
  case IL2CPP::IL2CPP_TYPE_R8:
    return 8;
  case IL2CPP::IL2CPP_TYPE_VALUETYPE: {
    auto klass = Class(type).GetClass();
    return klass ? klass->minimumAlignment : sizeof(void *);
  }
  default:
    return sizeof(void *);
  }
...
static void ModifyClass(MANAGEMENT_STRUCTURES::CustomClass *customClass,
  ...
  if (newMethodsCount) {
    ...
    for (size_t i = 0; i < newMethodsCount; ++i) {
      ...
      BNM_LOG_DEBUG_IF(
          ...
          MethodBase(method->_origin).str().c_str());
    }

    if (!methodsToAdd.empty()) {
      auto oldMethods = klass->methods;
      auto newMethods = (IL2CPP::MethodInfo **)BNM_MALLOC_TRACKED(
          (oldCount + methodsToAdd.size()) * sizeof(IL2CPP::MethodInfo *));

      auto oldSize = oldCount * sizeof(IL2CPP::MethodInfo *);

      if (oldCount && oldMethods)
        memcpy(newMethods, oldMethods, oldSize);

      memcpy(newMethods + oldCount, methodsToAdd.data(),
             methodsToAdd.size() * sizeof(IL2CPP::MethodInfo *));

      // Track old array for cleanup at shutdown if it was allocated by us
      if ((klass->flags & BNM_CLASS_ALLOCATED_METHODS_FLAG) ==
          BNM_CLASS_ALLOCATED_METHODS_FLAG) {
      ...
      // Atomic update of the methods array pointer using std::atomic_ref
      // (C++20)
      std::atomic_ref<const IL2CPP::MethodInfo **> methodsRef(klass->methods);
      methodsRef.store((const IL2CPP::MethodInfo **)newMethods,
                       std::memory_order_release);

      // Atomic update of the method count
      std::atomic_ref<uint16_t> countRef(klass->method_count);
      countRef.store((uint16_t)(oldCount + methodsToAdd.size()),
                     std::memory_order_release);
    }
  }
  ...
  if (newFieldsCount) {
    ...
    auto newFields = (IL2CPP::FieldInfo *)BNM_MALLOC_TRACKED(
        (oldCount + newFieldsCount) * sizeof(IL2CPP::FieldInfo));

    if (oldCount && oldFields)
      memcpy(newFields, oldFields, oldCount * sizeof(IL2CPP::FieldInfo));
    ...
    for (auto field : customClass->_fields) {
      ...
      // Alignment: Adjust currentAddress to the field's natural alignment
      size_t alignment = GetTypeAlignment(newField->type);
    ...
    // Track old array for cleanup at shutdown if it was allocated by us
    if ((klass->flags & BNM_CLASS_ALLOCATED_FIELDS_FLAG) ==
        BNM_CLASS_ALLOCATED_FIELDS_FLAG) {
    ...
    // Atomic update of the fields array pointer using std::atomic_ref (C++20)
    std::atomic_ref<IL2CPP::FieldInfo *> fieldsRef(klass->fields);
    fieldsRef.store(newFields, std::memory_order_release);

    // Atomic update of the field count
    std::atomic_ref<uint16_t> countRef(klass->field_count);
    countRef.store((uint16_t)(oldCount + newFieldsCount),
                   std::memory_order_release);
  }
...
static void CreateClass(MANAGEMENT_STRUCTURES::CustomClass *customClass,
  ...
  if (classInfo._imageName) {
    ...
    for (auto assembly : assemblies) {
      auto currentImage =
          Internal::il2cppMethods.il2cpp_assembly_get_image(assembly);
      if (!Internal::CompareImageName(currentImage, classInfo._imageName))
        continue;
      image = currentImage;
      break;
    }
  ...
  if (!image)
    image = MakeImage(classInfo._imageName);

  // Check if class already exists in the target image to prevent duplicate
  // registration
  if (auto existingClass = Internal::TryGetClassInImage(
          image._data, classInfo._namespace ? classInfo._namespace : "",
          classInfo._name)) {
    BNM_LOG_WARN("BNM: Class %s::%s already exists in image %s. Switching to "
                 "ModifyClass instead of CreateClass.",
                 classInfo._namespace ? classInfo._namespace : "",
                 classInfo._name, image._data->name);
    ModifyClass(customClass, existingClass);
    return;
  }

  BNM_LOG_DEBUG(DBG_BNM_MSG_ClassesManagement_CreateClass_Target,
                classInfo._namespace ? classInfo._namespace : &forEmptyString,
                classInfo._name, image._data->name);

  IL2CPP::Il2CppClass *parent = customClass->_baseType;
  if (!parent)
    parent = Internal::vmData.Object;
  ...
  for (size_t i = 0; i < customClass->_methods.size(); ++i) {
    ...
    if (!method->_isStatic)
      for (uint16_t v = 0; v < newVtableSize; ++v) {
        ...
        if (!strcmp(vTable.method->name, method->myInfo->name) &&
            count == method->myInfo->parameters_count &&
            method->_parameterTypes.size() == count) {
          if (!method->_skipTypeMatch)
            for (uint8_t p = 0; p < count; ++p) {
              ...
              if (Class(type).GetClass() !=
                  method->_parameterTypes[p].ToIl2CppClass())
                goto NEXT;
            }

          if (!hasFinalize && Internal::finalizerSlot != -1)
            hasFinalize = (int32_t)v == Internal::finalizerSlot;
          method->_origin = (IL2CPP::MethodInfo *)vTable.method;
          method->_originalAddress =
              (void *)(vTable.method ? vTable.method->methodPointer : nullptr);
          method->myInfo->slot = v;
          vTable.method = method->myInfo;
          vTable.methodPtr = method->myInfo->methodPointer;

          BNM_LOG_DEBUG(
              DBG_BNM_MSG_ClassesManagement_CreateClass_Overridden_Method,
              MethodBase(method->_origin).str().c_str());
  ...
  if (!owner && classInfo._namespace) {
    ...
    klass->namespaze = &forEmptyString;
  ...
  // and make type PUBLIC TYPE_ATTRIBUTE_ABSTRACT, TYPE_ATTRIBUTE_INTERFACE,
  // TYPE_ATTRIBUTE_PUBLIC
  klass->flags =
      (klass->parent->flags & ~(0x00000080 | 0x00000020)) | 0x0000001;
  ...
  for (size_t i = 0; i < newVTable.size(); ++i)
    klass->vtable[i] = newVTable[i];
  ...
  // BNM creates only classes, because new structs are not particularly needed
  // for modifications
  klass->valuetype = 0;
#endif

  // BNM does not support creating a static field constructor (.cctor). Il2Cpp
  // is designed so that it will never be called, and BNM does not allow you to
  ...
  klass->initializationExceptionGCHandle =
      (decltype(klass->initializationExceptionGCHandle))0;
...
typedef void (*ProcessorType)(CompileTimeClass &target,
                              CompileTimeClass::_BaseInfo *info);
...
static CustomClassInfo GetClassInfo(const CompileTimeClass &compileTimeClass) {
  ...
  do {
    ...
    // Protection from other types. We are not interested in Generic, Modifier,
    // and other things that do not relate to the class hierarchy.
    if (index != (uint8_t)CompileTimeClass::_BaseType::Class) {
      current = current->next;
      continue;
    }

    CompileTimeClassProcessors::processors[index](
        tmp, (CompileTimeClass::_BaseInfo *)info);
    ...
    if (found)
      return {classInfo->_namespace, classInfo->_name, classInfo->_imageName};
    return {};
...
static IL2CPP::Il2CppImage *MakeImage(std::string_view imageName) {
  ...
  if (imageName.ends_with(dotDllString))
    imageName.remove_suffix(4);
  ...
  auto handle = (IL2CPP::Il2CppImageDefinition *)BNM_MALLOC_TRACKED(
      sizeof(IL2CPP::Il2CppImageDefinition));
  ...
  newImg->nameToClassHashTable =
      (decltype(newImg->nameToClassHashTable))Internal::bnmImageSentinel;
...
static IL2CPP::VirtualInvokeData *
TryFindVirtualMethod(Class target, IL2CPP::MethodInfo *targetMethod,
                     uint16_t *outSlot = nullptr) {
  for (uint16_t i = 0; i < target._data->vtable_count; ++i) {
    auto &it = target._data->vtable[i];
    if (it.method != targetMethod)
      continue;
    if (outSlot)
      *outSlot = i;
    return &it;
...
ProcessCustomMethod(MANAGEMENT_STRUCTURES::CustomMethod *method, Class target,
  ...
  auto originalMethod = Internal::IterateMethods(
      target, [method, parameters](IL2CPP::MethodInfo *klassMethod) {
        if (method->_name != klassMethod->name ||
            klassMethod->parameters_count != parameters)
          return false;
        if (method->_skipTypeMatch)
          return true;
        ...
        for (uint8_t i = 0; i < parameters; ++i) {
          ...
          if (Class(param).GetClass() !=
              method->_parameterTypes[i].ToIl2CppClass())
            return false;
        }
        return true;
      });
  ...
  if (!originalMethod ||
      PRIVATE_INTERNAL::GetMethodClass(originalMethod) != target._data) {
    bool isVirtual =
        originalMethod != nullptr && (originalMethod->flags & 0x0040) == 0x0040;
    auto parent = originalMethod;
    originalMethod = CreateMethod(method);

    // Copy token to copy attributes
    if (!method->_copyTarget.empty()) {
      auto copyTarget = Internal::IterateMethods(
          target,
          [targetName = method->_copyTarget](IL2CPP::MethodInfo *method) {
            return targetName == method->name;
          });
      if (copyTarget)
        originalMethod->token = copyTarget->token;
    }
    ...
    bool canDoVirtualHook =
        hooked && isVirtual /* Can be true only if parent not null */;
    ...
    uint16_t slot{};
    if (auto vTable = TryFindVirtualMethod(target, parent, &slot);
        vTable != nullptr) {
      method->_origin = parent;
      method->_originalAddress = (void *)parent->methodPointer;
...
#undef kls
  ...
  if ((originalMethod->flags & 0x0040) == 0 || method->_isBasicHook)
    goto SKIP_NON_VIRTUAL;

  if (auto vTable = TryFindVirtualMethod(target, originalMethod);
      vTable != nullptr) {
    method->_origin = (IL2CPP::MethodInfo *)vTable->method;
...
SKIP_NON_VIRTUAL:
  ...
  method->_origin = originalMethod;
  ::BasicHook((void *)originalMethod->methodPointer, method->_address,
              method->_originalAddress);
...
CreateMethod(MANAGEMENT_STRUCTURES::CustomMethod *method) {
...
#if UNITY_VER >= 212
  myInfo->virtualMethodPointer =
      (decltype(myInfo->virtualMethodPointer))method->_address;
#endif
  ...
  if (method->_isStatic)
    myInfo->flags |= 0x0010; // |= STATIC
  if (method->_name == Internal::constructorName)
    myInfo->flags |= 0x0800 | 0x1000; // |= SPECIAL_NAME | RT_SPECIAL_NAME
  ...
  if (!methodType)
    methodType = Internal::vmData.Object;
  ...
  if (argsCount) {
...
#if UNITY_VER < 212
    myInfo->parameters = (IL2CPP::ParameterInfo *)BNM_MALLOC_TRACKED(
        argsCount * sizeof(IL2CPP::ParameterInfo));
    ...
    for (uint8_t p = 0; p < argsCount; ++p) {
      ...
      // Set the type anyway
      auto type =
          p < types.size() ? types[p].ToClass() : Internal::vmData.Object;
      if (!type)
        type = Internal::vmData.Object;
    ...
    auto parameters = (IL2CPP::Il2CppType **)BNM_MALLOC_TRACKED(
        argsCount * sizeof(IL2CPP::Il2CppType *));
...
static void SetupField(IL2CPP::FieldInfo *newField,
  ...
  if (!fieldType)
    fieldType = Internal::vmData.Object;
...
static void SetupClassParent(IL2CPP::Il2CppClass *target,
                             IL2CPP::Il2CppClass *parent) {
  // All C# classes should have parent. Only structs don't have it, but BNM
  // don't allow to define valid structs
  if (!parent) [[unlikely]]
    return;
  ...
  target->typeHierarchy = (IL2CPP::Il2CppClass **)BNM_MALLOC_TRACKED(
      target->typeHierarchyDepth * sizeof(IL2CPP::Il2CppClass *));
  memcpy(target->typeHierarchy, parent->typeHierarchy,
         parent->typeHierarchyDepth * sizeof(IL2CPP::Il2CppClass *));
  target->typeHierarchy[parent->typeHierarchyDepth] = target;
  target->parent = parent;
}

static void
GetAllInterfaces(IL2CPP::Il2CppClass *parent, IL2CPP::Il2CppClass *interface,
                 std::vector<IL2CPP::Il2CppClass *> &outInterfaces) { // NOLINT
  Internal::il2cppMethods.Class$$Init(interface);
  if (!HasInterface(parent, interface))
    outInterfaces.push_back(interface);
  if (!interface->interfaces_count ||
      interface->interfaces_count == (uint16_t)-1)
    return;

  for (uint16_t i = 0; i < interface->interfaces_count; ++i)
    GetAllInterfaces(parent, interface->implementedInterfaces[i],
                     outInterfaces);
}

static bool HasInterface(IL2CPP::Il2CppClass *parent,
                         IL2CPP::Il2CppClass *interface) { // NOLINT
  if (!parent || !interface)
    return false;
  for (uint16_t i = 0; i < parent->interfaces_count; ++i)
    if (parent->implementedInterfaces[i] == interface)
      return true;
  if (parent->parent)
    return HasInterface(parent->parent, interface);
  return false;
}
