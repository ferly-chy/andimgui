...
BNM::DelegateBase *BNM::DelegateBase::Create(BNM::MethodBase method) {
  return (BNM::DelegateBase *)BNM::Class(object.klass)
...
void BNM::MulticastDelegateBase::Add(BNM::DelegateBase *delegate) {
  ...
  if (!delegates) {
    auto arr =
        BNM::Class(this->delegate.object.klass).NewArray<DelegateBase *>(2);
    arr->m_Items[0] = (DelegateBase *)this;
    arr->m_Items[1] = delegate;
    this->delegates = (decltype(this->delegates))arr;
    return;
  }

  auto arr = BNM::Class(delegates->klass->element_class)
                 .NewArray<DelegateBase *>(delegates->capacity + 1);
...
void BNM::MulticastDelegateBase::Remove(BNM::DelegateBase *delegate) {
  ...
  auto delegates = (Structures::Mono::Array<DelegateBase *> *)this->delegates;
  if (!delegates || delegates->capacity == 0)
    return;

  IL2CPP::il2cpp_array_size_t index = 0;
  bool found = false;
  for (IL2CPP::il2cpp_array_size_t i = 0; i < delegates->capacity; ++i) {
    if (delegates->m_Items[i] != delegate)
      continue;
  ...
  auto arr = BNM::Class(delegates->klass->element_class)
                 .NewArray<DelegateBase *>(delegates->capacity - 1);
  if (index > 0)
    arr->CopyFrom(delegates->m_Items, index);
  if (index + 1 < delegates->capacity)
    memcpy(arr->m_Items + index, delegates->m_Items + index + 1,
           (delegates->capacity - index - 1) * sizeof(DelegateBase *));
  this->delegates = (decltype(this->delegates))arr;
}

BNM::DelegateBase *BNM::MulticastDelegateBase::Add(BNM::MethodBase method) {
  if (!CheckForNull(this) || !method.IsValid())
    return {};
