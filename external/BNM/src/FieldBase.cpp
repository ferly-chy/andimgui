...
FieldBase::FieldBase(IL2CPP::FieldInfo *info) {
  ...
  if (info->parent) {
    auto parentType = Class(info->parent).GetIl2CppType();
    _isInStruct =
        parentType && parentType->type == IL2CPP::IL2CPP_TYPE_VALUETYPE;
  }
}
...
FieldBase &FieldBase::SetInstance(IL2CPP::Il2CppObject *val) {
  ...
  if (BNM::IsA(val, _data->parent))
    _instance = val;
  else
    BNM_LOG_ERR(DBG_BNM_MSG_FieldBase_SetInstance_Wrong_Instance_Error,
...
void *FieldBase::GetFieldPointer() const {
  ...
  if (!_isStatic && !IsAllocated(_instance)) {
    BNM_LOG_ERR(
        DBG_BNM_MSG_FieldBase_GetFieldPointer_Error_instance_dead_instance,
        str().c_str());
    return nullptr;
  } else if (_isStatic && !IsAllocated(_data->parent)) {
    BNM_LOG_ERR(DBG_BNM_MSG_FieldBase_GetFieldPointer_Error_static_dead_parent,
                str().c_str());
    return nullptr;
  } else if (_isStatic && !_data->parent->static_fields) {
    BNM_LOG_ERR(DBG_BNM_MSG_FieldBase_GetFieldPointer_Error_static_dead_parent,
                str().c_str());
    return nullptr;
  } else if (_isThreadStatic) {
    BNM_LOG_ERR(
        DBG_BNM_MSG_FieldBase_GetFieldPointer_Error_thread_static_unsupported,
        str().c_str());
    return nullptr;
  } else if (_isConst) {
    BNM_LOG_ERR(DBG_BNM_MSG_FieldBase_GetFieldPointer_Error_const_impossible,
                str().c_str());
    return nullptr;
  }
  if (_isStatic)
    return (void *)((BNM_PTR)_data->parent->static_fields + _data->offset);
  return (void *)((BNM_PTR)_instance + _data->offset -
                  (_isInStruct ? sizeof(IL2CPP::Il2CppObject) : 0x0));
}
