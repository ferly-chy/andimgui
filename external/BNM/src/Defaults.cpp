...
ClassType Void{}, Boolean{}, Byte{}, SByte{}, Int16{}, UInt16{}, Int32{},
    UInt32{}, IntPtr{}, UIntPtr{}, Int64{}, UInt64{}, Single{}, Double{},
    Decimal{}, String{}, Object{};
ClassType Vector2{}, Vector3{}, Vector4{}, Color{}, Color32{}, Ray{},
    Quaternion{}, Matrix3x3{}, Matrix4x4{}, RaycastHit{}, RaycastHit2D{};
...
void BNM::Internal::LoadDefaults() {
  ...
  Vector2 =
      TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Vector2"));
  Vector3 =
      TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Vector3"));
  Vector4 =
      TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Vector4"));
  Color = TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Color"));
  Color32 =
      TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Color32"));
  Ray = TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Ray"));
  Quaternion = TryGetClassInImage(image, UnityEngineStr,
                                  BNM_OBFUSCATE_TMP("Quaternion"));
  Matrix3x3 =
      TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Matrix3x3"));
  Matrix4x4 =
      TryGetClassInImage(image, UnityEngineStr, BNM_OBFUSCATE_TMP("Matrix4x4"));

  RaycastHit = TryGetClassInImage(
      TryGetImage(BNM_OBFUSCATE_TMP("UnityEngine.PhysicsModule.dll")),
      UnityEngineStr, BNM_OBFUSCATE_TMP("RaycastHit"));

  RaycastHit2D = TryGetClassInImage(
      TryGetImage(BNM_OBFUSCATE_TMP("UnityEngine.Physics2DModule.dll")),
      UnityEngineStr, BNM_OBFUSCATE_TMP("RaycastHit2D"));

  // Unity
  UnityObject = TryGetClassInImage(image, UnityEngineStr, ObjectStr);
  MonoBehaviour = TryGetClassInImage(image, UnityEngineStr,
                                     BNM_OBFUSCATE_TMP("MonoBehaviour"));
}

BNM::Defaults::DefaultTypeRef::operator BNM::CompileTimeClass() const {
  return {_reference};
}
BNM::Defaults::DefaultTypeRef::operator BNM::Class() const {
  return _reference ? *_reference : nullptr;
}
BNM::Class BNM::Defaults::DefaultTypeRef::ToClass() const {
  return _reference ? *_reference : nullptr;
}