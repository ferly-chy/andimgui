...
void *RaycastHit::GetCollider() const {
  if (!m_Collider || (BNM_PTR)m_Collider < 0)
    return {};
#if UNITY_VER > 174
#if UNITY_VER >= 232
...
const Vector4 Vector4::positiveInfinity = {floatInf, floatInf, floatInf,
                                           floatInf};
const Vector4 Vector4::negativeInfinity = {-floatInf, -floatInf, -floatInf,
                                           -floatInf};
...
BNM::Class UnityEventBase::GetArgumentType(PersistentCall *call) {
  ...
  if (!typeName->IsNullOrEmpty())
    if (auto t = Internal::vmData.Type$$GetType(typeName); t != nullptr)
      type = t;
...
BNM::Class UnityEventBase::GetTargetType(PersistentCall *call) {
  ...
  if (call->m_Target != nullptr)
    targetType = call->m_Target;
  else
    targetType = Internal::vmData.Type$$GetType(call->m_TargetAssemblyTypeName);
