...
/**
    @brief Namespace that holds some basic and the most commonly used C#
   classes.
...
template <typename T> void InitMonoListVTable(List<T> *list);
} // namespace PRIVATE_MonoListData
/// @endcond
...
struct String : BNM::IL2CPP::Il2CppObject {
  ...
  /**
      ...
      @return Converted string if it's valid, otherwise "ERROR: Mono::String
     dead".
  */
  std::string str();
...
*/
template <typename T> struct Array : BNM::IL2CPP::Il2CppObject {
  ...
  */
  ...
  /**
      ...
      @return Array size if it's valid, otherwise zero.
  */
  [[nodiscard]] inline IL2CPP::il2cpp_array_size_t GetSize() const {
    return GetCapacity();
  }
  ...
  */
  ...
  /**
      ...
      @return DataIterator of target element if array is valid and index isn't
     out of bounds, otherwise empty DataIterator.
  */
  ...
  inline Utils::DataIterator<T>
  operator[](IL2CPP::il2cpp_array_size_t index) const {
    return At(index);
  }
  ...
  */
  ...
  /**
      ...
      @param _forceUseAlloc Forces code to create array using BNM::Allocate
     instead of BNM::Class::NewArray (Advanced usage only).
...
/**
    ...
    @tparam T Element type
*/
template <typename T> struct List : BNM::IL2CPP::Il2CppObject {
  ...
  */
  struct Enumerator {
    ...
    [[nodiscard]] inline T *end() const {
      return &list->items->m_Items[list->size];
    }
  };
  ...
  */
  [[nodiscard]] inline int GetCapacity() const {
    return items ? items->GetCapacity() : 0;
  }
  ...
  */
  void Add(T val) {
    ...
    if constexpr (std::is_move_assignable_v<T>) {
      items->m_Items[size] = std::move(val);
    } else {
  ...
  */
  bool Resize(int newCapacity) {
    BNM_CHECK_SELF(false);
    if (newCapacity <= (int)items->capacity)
      return false;
    ...
    if (size > 0) {
      if constexpr (std::is_trivially_copyable_v<T>) {
        ...
        for (int i = 0; i < size; ++i)
          nItems->m_Items[i] = std::move(items->m_Items[i]);
      }
    }
    items = nItems;
    return true;
  ...
  /**
      ...
      @return DataIterator of target element if index isn't out of bounds,
  ...
  void Clear() {
    if (size > 0) {
      if constexpr (std::is_trivially_destructible_v<T>) {
        memset(items->m_Items, 0, size * sizeof(T));
      } else {
        for (int i = 0; i < size; ++i)
          items->m_Items[i] = T();
      }
  ...
  */
  ...
  void *get_SyncRoot() {
    if (!syncRoot)
      syncRoot = PRIVATE_MonoListData::CompareExchange4List(syncRoot);
    return syncRoot;
  }
  ...
  */
  void Shift(int start, int delta) {
    ...
    // delta < 0 removes elements, delta > 0 opens a gap before insertion.
    if (delta < 0) {
      int removeCount = -delta;
      if (start < 0 || start + removeCount > size)
        return;

      int from = start + removeCount;
      int count = size - from;
      if (count > 0) {
        if constexpr (std::is_trivially_copyable_v<T>) {
          memmove(items->m_Items + start, items->m_Items + from,
                  count * sizeof(T));
        } else {
          for (int i = 0; i < count; ++i)
            items->m_Items[start + i] = std::move(items->m_Items[from + i]);
        }
      }
      size -= removeCount;
      if constexpr (std::is_trivially_destructible_v<T>) {
        memset(items->m_Items + size, 0, removeCount * sizeof(T));
      } else {
        for (int i = 0; i < removeCount; ++i)
          items->m_Items[size + i] = T();
      }
      return;
    }
    ...
    if (start < size) {
      if constexpr (std::is_trivially_copyable_v<T>) {
        memmove(items->m_Items + start + delta, items->m_Items + start,
                (size - start) * sizeof(T));
      } else {
        for (int i = size - 1; i >= start; --i)
          items->m_Items[i + delta] = std::move(items->m_Items[i]);
      }
    }
    size += delta;
  }
...
/// @cond
namespace PRIVATE_MonoListData {
template <typename T> constexpr std::string_view WrappedTypeName() {
...
constexpr std::size_t WrappedTypeNameSuffixLength() {
  return WrappedTypeName<void>().length() - WrappedTypeNamePrefixLength() - 4;
}
...
template <typename T> constexpr uint32_t HashedTypeName() {
  ...
  constexpr auto typeNameLength =
      wrappedTypeName.length() - prefixLength - suffixLength;
  constexpr auto typeName =
      wrappedTypeName.substr(prefixLength, typeNameLength);
...
IL2CPP::Il2CppClass *TryGetMonoListClass(uint32_t typeHash,
                                         std::array<MethodData, 16> &data);

template <typename T> void InitMonoListVTable(List<T> *list) {
  using namespace PRIVATE_MonoListData;
  /*
   * Replaces the virtual method table because the original one is empty.
   * It is also necessary for types that are not in the game (it's not a fact
   * that the game would be able to use them without that types, but the code is
   * there)
   */
  ...
  constexpr auto get_Item = &List<Type>::get_Item;
  constexpr auto set_Item = &List<Type>::set_Item;
  constexpr auto IndexOf = &List<Type>::IndexOf;
  constexpr auto Insert = &List<Type>::Insert;
  constexpr auto get_false = &List<Type>::get_false;
  ...
  constexpr auto get_SyncRoot = &List<Type>::get_SyncRoot;
  static std::array<MethodData, 16> namesMap = {
      MethodData{BNM_OBFUSCATE("RemoveAt"), *(void **)&RemoveAt},
      MethodData{BNM_OBFUSCATE("get_Count"), *(void **)&GetSize},
      MethodData{BNM_OBFUSCATE("Clear"), *(void **)&Clear},
      MethodData{BNM_OBFUSCATE("get_Item"), *(void **)&get_Item},
      MethodData{BNM_OBFUSCATE("set_Item"), *(void **)&set_Item},
      MethodData{BNM_OBFUSCATE("IndexOf"), *(void **)&IndexOf},
      MethodData{BNM_OBFUSCATE("Insert"), *(void **)&Insert},
      MethodData{BNM_OBFUSCATE("get_IsReadOnly"), *(void **)&get_false},
      MethodData{BNM_OBFUSCATE("get_IsFixedSize"), *(void **)&get_false},
      MethodData{BNM_OBFUSCATE("get_IsSynchronized"), *(void **)&get_false},
      MethodData{BNM_OBFUSCATE("Add"), *(void **)&Add},
      MethodData{BNM_OBFUSCATE("Contains"), *(void **)&Contains},
      MethodData{BNM_OBFUSCATE("CopyTo"), *(void **)&CopyTo},
      MethodData{BNM_OBFUSCATE("Remove"), *(void **)&Remove},
      MethodData{BNM_OBFUSCATE("GetEnumerator"), *(void **)&GetEnumerator},
      MethodData{BNM_OBFUSCATE("get_SyncRoot"), *(void **)&get_SyncRoot}};
