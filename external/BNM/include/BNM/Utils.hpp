...
/**
    @brief Macro function for checking pointer for null. Due to noinline
   attribute allows to check even `this` of objects.
*/
template <typename T> NO_INLINE bool CheckForNull(T obj) { return (void *)obj; }
...
inline bool IsAllocated(T x) {
  ...
  if (nullFd != -1) {
    // write() will return -1 and set errno to EFAULT if the pointer is invalid,
...
/**
    @brief Macro function for creating C# strings
   (BNM::Structures::Mono::String).
...
/**
    ...
    @return Dlfcn handle of libil2cpp.so if it's used by BNM and BNM is loaded,
   otherwise null.
*/
...
/**
    ...
    @return IL2CPP::Il2CppThread if current tread is attached to VM, otherwise
   null.
*/
...
template <typename T> inline T ReturnEmpty() {
...
/**
    ...
    Has only pointer to last element, but elements are looped, so last element
   points to first one. It used internally, so it lacks a lot of C++ stuff like
   foreach.
*/
