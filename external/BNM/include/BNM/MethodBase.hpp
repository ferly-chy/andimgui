...
/**
    ...
    This class provides fields and some basic functions to allow to call
   methods.
*/
struct MethodBase {
  /**
  ...
  */
  inline BNM_PTR GetOffset() const {
    return _data ? (BNM_PTR)_data->methodPointer : 0;
  }

  /**
      @brief Get typed version of current generic method.

      @return Typed method if method is valid and generic, otherwise empty
     method.
  */
  MethodBase GetGeneric(
      const std::initializer_list<CompileTimeClass> &templateTypes) const;

  /**
      @brief Get overridden method of current virtual method.
      @return Overridden method if method is valid and virtual, otherwise empty
     method.
  */
  ...
  */
  ...
  /**
      ...
      @return Reference to current MethodBase
  */
  inline MethodBase &operator[](void *instance) {
  ...
  /**
      ...
      @return Reference to current MethodBase
  */
  inline MethodBase &operator[](IL2CPP::Il2CppObject *instance) {
  ...
  /**
      ...
      @return Reference to current MethodBase
  */
  inline MethodBase &operator[](UnityEngine::Object *instance) {
    SetInstance((IL2CPP::Il2CppObject *)instance);
    return *this;
  }
  ...
  */
  ...
  /**
      ...
      Returns string with full method name: ClassFullName.(method
     name){arguments count: ...} (static)/nothing

      @return Method's full name or "Dead method".
  */
  inline std::string str() const {
    if (!_data)
      return BNM_OBFUSCATE(DBG_BNM_MSG_MethodBase_str_nullptr);
    return Class(_data->return_type).str() + BNM_OBFUSCATE(" ") +
           Class(BNM::PRIVATE_INTERNAL::GetMethodClass(_data)).str() +
           BNM_OBFUSCATE(".(") + _data->name +
           BNM_OBFUSCATE("){" DBG_BNM_MSG_MethodBase_str_args_count ": ") +
           std::to_string(_data->parameters_count) + BNM_OBFUSCATE("}") +
           (_isStatic ? BNM_OBFUSCATE("(" DBG_BNM_MSG_MethodBase_str_static ")")
                      : BNM_OBFUSCATE(""));
  }
#endif
  ...
  */
  template <typename NewType> inline Method<NewType> &cast() const {
    return (Method<NewType> &)*this;
  }
...
/**
    ...
    There are not many such methods in Unity: All unity messages (like Start,
   Update etc.), some of system actions. Primary usage is Unity messages,
   because this way of hooking don't add any overhead, that basic hooks do.

    @param info Target method info
    @param newMet Pointer to replacement method
    @param oldMet Pointer where original method pointer will be stored. Can be
   null
...
bool InvokeHook(const BNM::MethodBase &targetMethod, T_NEW newMet,
                T_OLD &oldMet) {
  if (targetMethod.IsValid())
    return InvokeHookImpl(targetMethod._data, (void *)newMet, (void **)&oldMet);
  return false;
}
...
/**
    ...
    This way of hooking will work only if virtual method called from some
   method. This hook won't be triggered by overridden methods, because that
   methods call overridden method directly, without class virtual table.
...
*/
bool VirtualHookImpl(BNM::Class targetClass, IL2CPP::MethodInfo *info,
                     void *newMet, void **oldMet);
...
*/
template <typename T_NEW, typename T_OLD>
bool VirtualHook(BNM::Class targetClass, const BNM::MethodBase &targetMethod,
                 T_NEW newMet, T_OLD &oldMet) {
  if (targetClass && targetMethod.IsValid())
    return VirtualHookImpl(targetClass, targetMethod._data, (void *)newMet,
                           (void **)&oldMet);
  return false;
}
...
*/
template <typename T_NEW, typename T_OLD>
bool VirtualHook(BNM::Class targetClass, IL2CPP::MethodInfo *info, T_NEW newMet,
                 T_OLD &oldMet) {
...
void BasicHook(const BNM::MethodBase &targetMethod, NEW_T newMethod,
               T_OLD &oldBytes) {
  if (targetMethod.IsValid())
    ::BasicHook((void *)targetMethod._data->methodPointer, newMethod, oldBytes);
}
