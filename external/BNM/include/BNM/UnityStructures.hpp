...
struct Object : public BNM::IL2CPP::Il2CppObject {
  ...
  /**
      ...
      @attention To check Unity object for null <b>ALWAYS</b> use this method.
     It checks object pointer and pointer to Unity object. These checks reduces
  ...
  */
  [[nodiscard]] inline bool IsValid() const __attribute__((always_inline)) {
    return BNM::IsAllocated(this) && this->klass != nullptr && m_CachedPtr != 0;
  }

  /**
      @brief Alias for IsValid()
  */
  [[nodiscard]] inline bool Alive() const __attribute__((always_inline)) {
    return IsValid();
  }
...
/**
    ...
    Should be used as parent in new classes if you are trying to create your own
   MonoBehaviour class.
*/
...
struct UnityAction : public MulticastDelegate<void> {
  inline void Invoke(Parameters... parameters) {
    (MulticastDelegate<void>(*this)).Invoke(parameters...);
  }
};
...
struct PersistentCall : public BNM::IL2CPP::Il2CppObject {
  ...
  [[nodiscard]] inline bool IsValid() const {
    return m_TargetAssemblyTypeName && m_TargetAssemblyTypeName->length &&
           m_MethodName && m_MethodName->length;
  }
};
...
*/
template <typename... Parameters> struct UnityEvent : public UnityEventBase {
  ...
  */
  inline void AddListener(UnityAction<Parameters...> *action) {
    if (!_AddListener_method.IsValid())
      _AddListener_method =
          BNM::Class(klass).GetMethod(BNM_OBFUSCATE("AddListener"), 1);
  ...
  */
  inline void RemoveListener(UnityAction<Parameters...> *action) {
    if (!_RemoveListener_method.IsValid())
      _RemoveListener_method =
          BNM::Class(klass).GetMethod(BNM_OBFUSCATE("RemoveListener"), 1);
  ...
  */
  void InvokePersistent(Parameters... parameters) {
    ...
    if (!m_PersistentCalls)
      return;
    if (calls = m_PersistentCalls->m_Calls; !calls)
      return;
    ...
    for (int i = 0; i < calls->size; ++i) {
      ...
      case PersistentListenerMode::Object:
        method = targetType.GetMethod(
            methodName,
            {argumentType.Alive() ? argumentType : Defaults::Get<Object *>()});
        break;
      ...
      case PersistentListenerMode::String:
        method = targetType.GetMethod(
            methodName, {Defaults::Get<Structures::Mono::String *>()});
        break;
      case PersistentListenerMode::Bool:
        method = targetType.GetMethod(methodName, {Defaults::Get<bool>()});
        break;
      }
      if (!method.IsValid() || (!method._isStatic && !persistentCall->m_Target))
        continue;

      if (!method._isStatic)
        method.SetInstance(persistentCall->m_Target);
  ...
  */
  inline void InvokeCalls(Parameters... parameters) {
    ...
    InvokeList((Structures::Mono::List<InvokableCall<Parameters...> *> *)
                   m_Calls->m_PersistentCalls,
               parameters...);
    InvokeList((Structures::Mono::List<InvokableCall<Parameters...> *> *)
                   m_Calls->m_RuntimeCalls,
               parameters...);
  }

private:
  void InvokeList(Structures::Mono::List<InvokableCall<Parameters...> *> *list,
