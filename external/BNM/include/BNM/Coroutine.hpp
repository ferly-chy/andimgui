...
/**
    @brief Class similar to UnityEngine.YieldInstruction, but not it's
   implementation.

    It's used as base class for custom BNM based YieldInstruction.
*/
struct YieldInstruction {
  inline YieldInstruction(BNM::IL2CPP::Il2CppObject *object)
      : _object(object) {};
...
/**
    @brief Analog of C# IEnumerator that is based on C++20 coroutines to emulate
   work of Unity's coroutines.
*/
struct IEnumerator : BNM::IL2CPP::Il2CppObject {
  ...
  struct promise_type {
    ...
    inline IEnumerator get_return_object() {
      return IEnumerator(
    ...
    yield_value(const Coroutine::YieldInstruction &val) {
      _currentValue = val;
      return {};
    }
...
private:
  ...
  explicit IEnumerator(std::coroutine_handle<promise_type> handle)
      : BNM::IL2CPP::Il2CppObject(), _coroutine(handle) {}
