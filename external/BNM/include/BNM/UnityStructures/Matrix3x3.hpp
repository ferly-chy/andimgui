...
inline static bool CompareApproximately(float f0, float f1,
...
struct Matrix3x3 {
  ...
  inline Matrix3x3(float m00, float m01, float m02, float m10, float m11,
  ...
  [[nodiscard]] inline const float &Get(int row, int column) const {
  ...
  [[nodiscard]] inline Vector3 GetColumn(int col) const {
    return {Get(0, col), Get(1, col), Get(2, col)};
  }

  inline static void MultiplyMatrices3x3(const Matrix3x3 *__restrict lhs,
                                         const Matrix3x3 *__restrict rhs,
                                         Matrix3x3 *__restrict res) {
    for (int i = 0; i < 3; ++i) {
      res->m_Data[i] = lhs->m_Data[i] * rhs->m_Data[0] +
                       lhs->m_Data[i + 3] * rhs->m_Data[1] +
                       lhs->m_Data[i + 6] * rhs->m_Data[2];
      res->m_Data[i + 3] = lhs->m_Data[i] * rhs->m_Data[3] +
                           lhs->m_Data[i + 3] * rhs->m_Data[4] +
                           lhs->m_Data[i + 6] * rhs->m_Data[5];
      res->m_Data[i + 6] = lhs->m_Data[i] * rhs->m_Data[6] +
                           lhs->m_Data[i + 3] * rhs->m_Data[7] +
                           lhs->m_Data[i + 6] * rhs->m_Data[8];
    }
  }
  ...
  MultiplyVector3Transpose(const Vector3 &inV) const {
    Vector3 res;
  ...
  [[nodiscard]] inline Vector3
  MultiplyPoint3Transpose(const Vector3 &inV) const {
    return MultiplyVector3Transpose(inV);
  }
  ...
  Matrix3x3 &operator*=(const Matrix3x3 &inM) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      Get(i, 0) =
          v[0] * inM.Get(0, 0) + v[1] * inM.Get(1, 0) + v[2] * inM.Get(2, 0);
      Get(i, 1) =
          v[0] * inM.Get(0, 1) + v[1] * inM.Get(1, 1) + v[2] * inM.Get(2, 1);
      Get(i, 2) =
          v[0] * inM.Get(0, 2) + v[1] * inM.Get(1, 2) + v[2] * inM.Get(2, 2);
    }
    return *this;
  }
  ...
  [[nodiscard]] inline float GetDeterminant() const {
    ...
    return fCofactor0 + fCofactor1 + fCofactor2 - fCofactor3 - fCofactor4 -
           fCofactor5;
  }
  ...
  inline Matrix3x3 &SetBasis(const Vector3 &inX, const Vector3 &inY,
                             const Vector3 &inZ) {
    Get(0, 0) = inX[0];
    Get(0, 1) = inY[0];
    Get(0, 2) = inZ[0];
  ...
  inline Matrix3x3 &SetBasisTransposed(const Vector3 &inX, const Vector3 &inY,
                                       const Vector3 &inZ) {
    Get(0, 0) = inX[0];
    Get(1, 0) = inY[0];
    Get(2, 0) = inZ[0];
  ...
  inline static bool LookRotationToMatrix(const Vector3 &viewVec,
                                          const Vector3 &upVec, Matrix3x3 *m) {
    Vector3 z = viewVec;
  ...
  inline static void GetRotMatrixNormVec(float *out, const float *inVec,
                                         float radians) {
    float s, c;
