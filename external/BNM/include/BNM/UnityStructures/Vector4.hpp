...
struct Vector4 {
  ...
  inline constexpr Vector4(float inX, float inY, float inZ, float inW)
      : x(inX), y(inY), z(inZ), w(inW) {}
  inline constexpr Vector4(Vector3 v, float inW)
  ...
  inline static float Component(Vector4 a, Vector4 b) {
    return Dot(a, b) / Magnitude(b);
  }
  inline static float Distance(Vector4 a, Vector4 b) {
    return Magnitude(a - b);
  }
  inline static float Dot(Vector4 lhs, Vector4 rhs) {
    return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z + lhs.w * rhs.w;
  }
  inline static bool IsFinite(Vector4 f) {
    return Unity::IsFinite(f.x) && Unity::IsFinite(f.y) &&
           Unity::IsFinite(f.z) && Unity::IsFinite(f.w);
  }
  inline static Vector4 Lerp(Vector4 from, Vector4 to, float t) {
    return to * t + from * (1.0f - t);
  }
  inline static Vector4 LerpUnclamped(Vector4 from, Vector4 to, float t) {
  ...
  inline static bool CompareApproximately(Vector4 a, Vector4 b,
    ...
    return SqrMagnitude(b - a) <= inMaxDist * inMaxDist;
  }
  ...
  inline friend Vector4 operator*(Vector4 lhs, Vector4 rhs) {
    return {lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z, lhs.w * rhs.w};
  }
  inline friend Vector4 operator*(Vector4 v, float s) {
