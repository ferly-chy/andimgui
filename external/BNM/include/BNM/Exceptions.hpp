...
struct Exception {
  ...
  */
  inline constexpr Exception(BNM::IL2CPP::Il2CppException *exception)
      : _data(exception) {}
  ...
  */
  [[nodiscard]] inline std::string ClassName() const {
    return ((BNM::Structures::Mono::String *)_data->className)->str();
  }
  ...
  */
  [[nodiscard]] inline std::string Message() const {
    return ((BNM::Structures::Mono::String *)_data->message)->str();
  }
  ...
  */
  [[nodiscard]] inline IL2CPP::Il2CppObject *Data() const {
    return _data->_data;
  }
...
/// @cond
inline void
_TryInvokeWrapper(const std::function<void()> &baseCode,
                  const std::function<void(Exception)> &catchCode = {}) {
  if (auto ex = TryInvoke(baseCode); ex.IsValid() && catchCode)
    catchCode(ex);
}
/// @endcond
} // namespace BNM
...
/**
    ...
    Macro that allows to define a block of code to be tested for exception while
...
/**
    ...
    Macro that allows to define a block of code to be executed, if an exception
   occurs in the try macro block.
