...
/**
    ...
    Allow BNM to hook il2cpp::vm::Class::FromIl2CppType to load itself if
   loading starts too late. Should be called before attempting to load BNM.
   (Before @ref TryLoadByJNI, @ref TryLoadByDlfcnHandle, @ref
   TryLoadByUsersFinder and @ref TrySetupByUsersFinder).

    @warning For this loading method Unhook is required. Otherwise, the game
   will be slowed down.
*/
...
/**
    ...
    Uses JNI and Context to find the full path to libil2cpp.so and then hook
   methods for loading BNM from the il2cpp thread.
...
/**
    ...
    Uses the library handle to hook methods for loading BNM from the il2cpp
   thread.
...
/**
    ...
    You can use it for setting custom method finder if dlfcn or JNI don't work
   in current context.

    @param finderMethod Pointer to custom method finder
    @param userData Data that will be passed with every custom method finder
   call
*/
...
/**
    ...
    Uses custom method finder to hook methods for loading BNM from the il2cpp
   thread.
...
/**
   ...
   @warning You cannot call the method until il2cpp is fully loaded. It will
   cause crash.
   @warning Highly recommended not to call it from thread other than il2cpp's,
   it can cause a lot of problems and crashes, especially if classes management
   is enabled.
*/
void TrySetupByUsersFinder();

/**
    @brief Add event that will be called from il2cpp thread when il2cpp and BNM
