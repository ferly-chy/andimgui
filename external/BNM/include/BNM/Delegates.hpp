...
template <typename Ret> struct Delegate;
template <typename Ret> struct MulticastDelegate;
...
struct DelegateBase : public IL2CPP::Il2CppDelegate {
  ...
  */
  template <typename NewRet> inline Delegate<NewRet> &cast() const {
    return (Delegate<NewRet> &)*this;
  }
};
...
struct MulticastDelegateBase : public IL2CPP::Il2CppMulticastDelegate {
  ...
  /**
      ...
      @return Vector of MethodBase if delegate isn't null, otherwise empty
     vector.
  */
  ...
  */
  ...
  /**
      @brief Cast delegate to be able to invoke it.
  */
  template <typename NewRet> inline MulticastDelegate<NewRet> &cast() const {
    return (MulticastDelegate<NewRet> &)*this;
  }
};
...
*/
template <typename Ret> struct Delegate : public DelegateBase {
  ...
  */
  ...
  template <typename... Parameters>
  inline Ret operator()(Parameters... parameters) {
...
struct MulticastDelegate : public MulticastDelegateBase {
  ...
  */
  ...
  inline Ret Invoke(Parameters... parameters) {
    ...
    if (!delegates)
      return ((Delegate<Ret> *)this)->Invoke(parameters...);

    auto size = delegates->GetCapacity();
    if (size == 0)
      return BNM::PRIVATE_INTERNAL::ReturnEmpty<Ret>();
